...
export const destinyMatrixResults = sqliteTable('destiny_matrix_results', {
...
export const destinyUserHistory = sqliteTable('destiny_user_history', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userId: text('user_id').notNull().default('default'),
  ...
  index('idx_destiny_history_user_time').on(table.userId, table.createdAt),
]);
