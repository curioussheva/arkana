...
export function ElementBanner({ element, advice, onPress }: ElementBannerProps) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        <Text style={[styles.label, { color: colors.textSecondary }]}>ELEMEN DOMINAN</Text>
        <Text style={[styles.element, { color: style.color }]}>{element}</Text>
        <Text style={[styles.description, { color: colors.textSecondary }]}>{advice}</Text>
        {onPress && (
...
const styles = StyleSheet.create({
  ...
  icon: { fontSize: 28 },
  content: { flex: 1 },
  label: { fontSize: FONT_SIZE.xs, fontWeight: '700', letterSpacing: 1, marginBottom: 2 },
  element: { fontSize: FONT_SIZE.xl, fontWeight: '800', marginBottom: 4 },
