    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
          <Text style={[styles.modalTitle, { color: colors.text }]}>Ekor Karma - Beban Masa Lalu</Text>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {analysis.pastLifeDebt && (
            <View style={[styles.block, { borderColor: colors.border }]}>
              <Text style={[styles.label, { color: colors.textSecondary }]}>⏳ Akar Hutang Karma (Masa Lalu)</Text>
              <Text style={[styles.text, { color: colors.textSecondary, fontStyle: 'italic' }]}>{analysis.pastLifeDebt}</Text>
            </View>
          )}
          ...
          {analysis.manifestation && (
            <View style={[styles.block, { borderColor: colors.border }]}>
              <Text style={[styles.label, { color: colors.primary }]}>👁️ Manifestasi Saat Ini</Text>
          ...
          {analysis.currentTriggers && (
            <View style={[styles.block, { borderColor: colors.error + '30', backgroundColor: colors.error + '05' }]}>
              <Text style={[styles.label, { color: colors.error }]}>⚠️ Jebakan Harian (Shadow Triggers)</Text>
              <Text style={[styles.text, { color: colors.text }]}>{analysis.currentTriggers}</Text>
            </View>
          )}
          ...
          {rumpun && (
            <View style={[styles.rumpunContainer, { backgroundColor: colors.border + '10', borderColor: colors.border }]}>
              <TouchableOpacity activeOpacity={0.7} onPress={() => setIsRumpunExpanded(!isRumpunExpanded)} style={styles.rumpunHeader}>
                <View style={{ flex: 1, marginRight: SPACING.sm }}>
                  <Text style={[styles.rumpunMetaLabel, { color: colors.textSecondary + 'aa' }]}>RUMPUN KETERKAITAN KARMIC</Text>
                  <Text style={[styles.rumpunName, { color: colors.primary }]}>{rumpun.icon} {rumpun.name}</Text>
                </View>
              ...
              {isRumpunExpanded && (
                <View style={styles.rumpunContent}>
                  <Text style={[styles.rumpunDescription, { color: colors.text }]}>{rumpun.description}</Text>
                  
                  {/* 📊 SEKSI BARU: Ringkasan Komparatif Intra Rumpun */}
                  {RUMPUN_COMPARATIVE_SUMMARY[rumpun.icon] && (
                    <View style={[styles.summaryBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
                      <Text style={[styles.summaryBoxTitle, { color: colors.textSecondary }]}>PETA KONTEKS PERBEDAAN ENERGI:</Text>
                  ...
                  <Text style={[styles.variationLabel, { color: colors.textSecondary + 'bb', marginTop: SPACING.md }]}>
                  ...
                  {finalVariations.map((v) => {
                    ...
                    return (
                      <View 
                        ...
                        style={[
                          styles.variationRow, 
                          isCurrent && { backgroundColor: colors.primary + '15', borderColor: colors.primary + '40' }
                        ]}
                      >
                        <View style={[styles.variationBadge, { backgroundColor: isCurrent ? colors.primary : colors.border }]}>
                          <Text style={[styles.variationBadgeText, { color: isCurrent ? colors.background : colors.textSecondary }]}>
                            {v.triad}
                          </Text>
                        </View>
                        <Text 
                          style={[
                            ...
                            { color: isCurrent ? colors.text : colors.textSecondary, fontWeight: isCurrent ? '700' : '400' }
                          ]} 
                          numberOfLines={1}
                        >
          ...
          {analysis.affirmation && (
            <View style={[styles.quoteBox, { backgroundColor: colors.primary + '08', borderColor: colors.primary + '40' }]}>
              ...
              <Text style={[styles.quoteText, { color: colors.text }]}>&ldquo;{analysis.affirmation}&rdquo;</Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  codeContainer: { alignSelf: 'center', borderRadius: BORDER_RADIUS.lg, paddingHorizontal: SPACING.lg, paddingVertical: SPACING.sm, marginBottom: SPACING.lg },
  code: { fontSize: FONT_SIZE.lg, fontWeight: '800', letterSpacing: 1 },
  title: { fontSize: FONT_SIZE.xl, fontWeight: '700', textAlign: 'center', marginBottom: SPACING.xl },
  block: { borderWidth: 1, borderRadius: BORDER_RADIUS.xl, padding: SPACING.md, marginBottom: SPACING.md },
  label: { fontSize: FONT_SIZE.sm, fontWeight: '700', marginBottom: SPACING.xs },
  text: { fontSize: FONT_SIZE.sm, lineHeight: 22 },
  quoteBox: { padding: SPACING.md, borderRadius: BORDER_RADIUS.xl, borderLeftWidth: 4, marginTop: SPACING.sm },
  quoteTitle: { fontSize: 11, fontWeight: '700', textTransform: 'uppercase', marginBottom: 4, letterSpacing: 0.5 },
  quoteText: { fontSize: FONT_SIZE.sm, fontStyle: 'italic', lineHeight: 22 },
  rumpunContainer: { borderWidth: 1, borderRadius: BORDER_RADIUS.xl, padding: SPACING.md, marginBottom: SPACING.md },
  ...
  rumpunName: { fontSize: FONT_SIZE.sm, fontWeight: '700' },
  rumpunContent: { marginTop: SPACING.md, borderTopWidth: 0.5, borderTopColor: '#00000015', paddingTop: SPACING.md },
  rumpunDescription: { fontSize: FONT_SIZE.xs, lineHeight: 18, marginBottom: SPACING.sm },
  summaryBox: { borderWidth: 1, borderRadius: BORDER_RADIUS.md, padding: SPACING.sm, marginVertical: SPACING.xs },
  summaryBoxTitle: { fontSize: 9, fontWeight: '700', letterSpacing: 0.5, marginBottom: 4 },
  summaryBoxText: { fontSize: 11, lineHeight: 16, fontFamily: 'monospace' },
  variationLabel: { fontSize: 10, fontWeight: '700', marginBottom: SPACING.sm },
  variationRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: 'transparent', borderRadius: BORDER_RADIUS.md, padding: 6, marginBottom: 4 },
  variationBadge: { paddingHorizontal: SPACING.sm, paddingVertical: 2, borderRadius: BORDER_RADIUS.sm, marginRight: SPACING.sm },
  variationBadgeText: { fontSize: 10, fontWeight: '700' },
