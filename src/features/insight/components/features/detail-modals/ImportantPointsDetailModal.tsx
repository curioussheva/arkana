...
import { DESTINY_MATRIX_MAP } from '@constants/images';
...
const POINT_GROUPS: Record<string, string[]> = {
  '🎯 Pilar Pusat (Esensi Takdir)': ['A', 'B', 'C', 'D', 'E'],
  '⭐ Jalur Langit (Sosial & Mental)': ['A1', 'A2', 'A3', 'B1', 'B2', 'B3'],
  '🌍 Jalur Bumi (Material & Finansial)': ['C1', 'C2', 'C3'],
  '🪞 Jalur Karma Masa Lalu': ['D1', 'D2', 'D3'],
  '💫 Klaster Personal & Ekor Karma': ['N', 'O', 'P'],
};

// 🎯 FIX 3: Penyelarasan Kamus Penjelasan Statis Posisi Kompas Jiwa
const COMPASS_LEGEND = [
  {
    title: '🎯 Pilar Pusat (Esensi Takdir)',
    position: 'Titik A, B, C, D, E',
    desc: 'Pondasi utama cetak biru takdir, karakter mental, spiritualitas batin, dan zona nyaman esensi jiwa Anda.',
  },
  {
    title: '⭐ Jalur Langit (Sosial & Mental)',
    position: 'Kutub Atas (A1, A2, A3) & Kanan (B1, B2, B3)',
    desc: 'Ekstensi energi yang mengatur alur ekspresi karakter sosial, komunikasi, potensi spiritual, dan perlindungan makro.',
  },
  {
    title: '🌍 Jalur Bumi (Material & Finansial)',
    position: 'Kutub Bawah (C1, C2, C3)',
    desc: 'Gerbang realisasi duniawi, karir, kemakmuran finansial, bisnis, akumulasi aset, serta pintu masuk roda arus rezeki.',
  },
  {
    title: '🪞 Jalur Karma Masa Lalu',
    position: 'Kutub Kiri (D1, D2, D3)',
    desc: 'Manifestasi tantangan, rintangan berulang, dan utang karma bawaan. Simpul pengunci utama yang wajib diselaraskan.',
  },
  {
    title: '💫 Klaster Personal & Ekor Karma',
    position: 'Jalur Transisi (N, O, P)',
    desc: 'Keseimbangan aliran energi leluhur serta titik simpul akhir penghubung transformasi mutasi alur takdir.',
  },
];
...
export function ImportantPointsDetailModal({ visible, data, onClose }: Props) {
  ...
  // 🃏 RESONANSI KARTU HARIAN
  const dailyResonancePoint = useMemo(() => {
    ...
    try {
      ...
      const match = data.find(item => {
        const itemArcanaId = item.arcana?.id ?? item.value ?? 0;
        const cleanId = itemArcanaId === 0 ? 22 : itemArcanaId;
        return cleanId === dailyId;
      });

      if (!match) return null;

      return {
        spatialKey: GEOMETRIC_KEY_MAP[match.key] || match.key,
        cardName: dailyCard.tarotName,
        cardId: dailyId,
  ...
  const groupedData = useMemo(() => {
    ...
    Object.entries(POINT_GROUPS).forEach(([groupName, allowedSpatialKeys]) => {
      ...
      matches.sort((a, b) =>
        a.spatialKey.localeCompare(b.spatialKey, undefined, { numeric: true, sensitivity: 'base' })
      );
  ...
  return (
    ...
    >
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          {/* ⚡ BANNER RESONANSI TRANSIT HARIAN */}
          {dailyResonancePoint && (
            <Animated.View
              entering={FadeInDown.duration(350)}
              style={[
                styles.resonanceBanner,
                { backgroundColor: colors.primary + '10', borderColor: colors.primary + '40' },
              ]}
            >
              <Text style={{ fontSize: 20 }}>⚡</Text>
              <View style={{ flex: 1 }}>
                <Text style={[styles.resonanceBannerTitle, { color: colors.primary }]}>
                  Aktivasi Transit Harian Terdeteksi!
                </Text>
                <Text style={[styles.resonanceBannerBody, { color: colors.textSecondary }]}>
                  Kartu Harian{' '}
                  <Text style={{ fontWeight: '800' }}>
                    {dailyResonancePoint.cardName} (#{dailyResonancePoint.cardId})
                  </Text>{' '}
                  menyinari titik koordinat{' '}
                  <Text style={{ fontWeight: '800', color: colors.text }}>
                    {dailyResonancePoint.spatialKey} ({dailyResonancePoint.label})
                  </Text>
                  .
                </Text>
              </View>
          ...
          <View style={[styles.imageContainer, { borderColor: colors.border }]}>
            <Image source={DESTINY_MATRIX_MAP} style={styles.matrixImage} resizeMode="contain" />
          </View>
          ...
          >
            <Text style={[styles.legendHeaderTitle, { color: colors.text }]}>
              🗺️ Panduan Koordinat Cetak Biru
            </Text>
            ...
            {COMPASS_LEGEND.map((item, index) => (
              ...
              >
                <View style={styles.legendRowTop}>
                  <Text style={[styles.legendRowTitle, { color: colors.primary }]}>
                    {item.title}
                  </Text>
                  <Text
                    style={[
                      styles.legendRowPosition,
                      { color: colors.textMuted, backgroundColor: colors.backgroundLight },
                    ]}
                  >
                    {item.position}
                  </Text>
                </View>
                <Text style={[styles.legendRowDesc, { color: colors.textSecondary }]}>
                  {item.desc}
                </Text>
              </View>
            ))}
          ...
          <Text style={[styles.title, { color: colors.text, marginTop: SPACING.xl }]}>
            🌱 Rencana Aksi Kontekstual
          </Text>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
            Ketuk klaster alur di bawah ini untuk membuka analisis mendalam, praktik harian, metode
            ritus, dan afirmasi murni takdir Anda.
          </Text>
          ...
          {Object.keys(groupedData).length > 0 ? (
            Object.entries(groupedData).map(([groupName, points]) => (
              <View key={groupName} style={styles.groupSection}>
                ...
                {points.map(point => {
                  ...
                  const isResonating = dailyResonancePoint?.spatialKey === point.spatialKey;
                    ...
                    <Animated.View
                      ...
                      style={[
                        styles.accordionCard,
                        {
                          ...
                          borderColor: isResonating ? colors.primary : colors.border,
                          borderLeftWidth: isResonating ? 4 : 1,
                        },
                      ]}
                    >
                      <TouchableOpacity
                        ...
                        <View style={styles.headerLeft}>
                          ...
                          <View style={{ flex: 1 }}>
                            ...
                            <Text
                              ...
                              numberOfLines={1}
                            >
                              #{displayId} - {arcana.tarotName} {isResonating ? '⚡' : ''}
                            </Text>
                          </View>
                        </View>
                        <Text
                      ...
                      {isExpanded && (
                        ...
                        >
                          ...
                          <View style={styles.subSection}>
                            <Text style={[styles.descriptionText, { color: colors.text }]}>
                              {point.interpretation ||
                                'Analisis getaran energi sedang diselaraskan dengan cetak biru Anda...'}
                            </Text>
                          </View>

                          {/* 2. Praktik Taktis Harian */}
                          {Array.isArray(arcana.dailyPractice) &&
                            arcana.dailyPractice.length > 0 && (
                              <View style={styles.subSection}>
                          ...
                          {Array.isArray(arcana.meditation) && arcana.meditation.length > 0 && (
                            <View style={styles.subSection}>
                              <Text style={[styles.subTitle, { color: colors.primary }]}>
                                🧘 Ritus & Metode Meditasi
                              </Text>
                          ...
                          {Array.isArray(arcana.affirmations) && arcana.affirmations.length > 0 && (
                            <View
                              style={[
                                styles.quoteBox,
                                {
            ...
            <Text
              style={[
                styles.bodyText,
                { textAlign: 'center', marginTop: SPACING.xl, color: colors.textMuted },
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  resonanceBannerTitle: { fontSize: 12, fontWeight: '800' },
  resonanceBannerBody: { fontSize: 11, marginTop: 1 },
