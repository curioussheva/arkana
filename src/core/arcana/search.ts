...
export function searchArcana(
  keyword: string,
): readonly ArcanaDefinition[] {
...
export function searchArcanaByKeywords(
...
export function searchArcanaByElement(
  element: string,
): ArcanaDefinition[] {
  const target = normalize(element);

  return ARCANA_DATABASE.filter(
    (arcana) => normalize(arcana.element) === target,
  );
}

export function searchArcanaByPlanet(
  planet: string,
): ArcanaDefinition[] {
  const target = normalize(planet);

  return ARCANA_DATABASE.filter(
    (arcana) => normalize(arcana.planet ?? "") === target,
  );
}

export function searchArcanaByZodiac(
  zodiac: string,
): ArcanaDefinition[] {
  const target = normalize(zodiac);

  return ARCANA_DATABASE.filter(
    (arcana) => normalize(arcana.zodiac ?? "") === target,
  );
}

export function searchArcanaByChakra(
  chakra: string,
): ArcanaDefinition[] {
  const target = normalize(chakra);

  return ARCANA_DATABASE.filter(
    (arcana) => normalize(arcana.chakra ?? "") === target,
  );
} 