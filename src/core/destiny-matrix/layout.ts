...
export const LAYOUT_RADII = {
  ...
  LM_CHANNEL: 0.39, // Saluran Uang & Cinta (Kanan-Bawah) — N/O/P
  SUB_INNER: 0.81, // Sub-nodes (Q-T / SubA..SubI)
  MAIN: 0.98, // Sudut Utama Matriks (posisi NODE, bukan garis oktagon)
  POWER_CENTER_1: 0.19, // familyCenter — berbaris ke kanan dari E (personalCenter)
  POWER_CENTER_2: 0.34, // unifiedCenter — lebih jauh lagi ke kanan, satu baris
  OCTAGON_BORDER: 1.1, // Garis rangka oktagon luar — SENGAJA lebih besar dari
  // MAIN, supaya ada celah antara tepi node dan garis
  // untuk ruang ruler timeline (lihat referensi Ladini)
  TIMELINE: 1.1, // Lingkaran Umur Luar — SAMA dengan OCTAGON_BORDER karena
...
export const POINT_LAYOUT: Record<string, PointCoordinate> = {
  ...
  LM_Center: polarToCartesian(315, LAYOUT_RADII.LM_CHANNEL * 1.3),

  P: polarToCartesian(335, LAYOUT_RADII.LM_CHANNEL * 1.35), // Bergeser ke arah C (Kanan)
  Money: polarToCartesian(340, LAYOUT_RADII.LM_CHANNEL * 1.4),

  O: polarToCartesian(295, LAYOUT_RADII.LM_CHANNEL * 1.4), // Bergeser ke arah D (Bawah)
  Love: polarToCartesian(290, LAYOUT_RADII.LM_CHANNEL * 1.4),
  ...
  // ─── 8. GREEN ZONE / TITIK HASRAT (Heart's Desire) ─────
  // Formula: reduce(J+E) & reduce(K+E) — dikonfirmasi dokumentasi Ladini
  // ("Green Zone"), BUKAN nama lain untuk E sendiri. Posisi: setengah jalan
  // antara A1/J (horizontal) atau B1/K (vertikal) menuju pusat E.
  HeartDesirePhysical: polarToCartesian(180, LAYOUT_RADII.INNER * 0.5), // reduce(J+E)
  HeartDesireSpiritual: polarToCartesian(90, LAYOUT_RADII.INNER * 0.5), // reduce(K+E)

  // ─── 9. 3 PUSAT KEKUATAN (Power Centers) ───────────────
  // personalCenter = E (sudah di posisi {x:0,y:0}, tidak perlu didaftar
  // ulang). FamilyCenter & UnifiedCenter berbaris ke kanan dari E, sesuai
  // tampilan kalkulator resmi (lingkaran menempel di pusat) — tervalidasi
  // 2 contoh tanggal lahir independen (12/08/1983 -> familyCenter=11;
  // 20/02/1967 -> familyCenter=9, keduanya cocok).
  // CATATAN: key PascalCase (FamilyCenter, bukan familyCenter) — harus
  // sama persis dengan DestinyPointKey di types.ts, karena itu nama key
  // asli di matrix.points (lihat rawMap di calculator/main.ts).
  FamilyCenter: polarToCartesian(0, LAYOUT_RADII.POWER_CENTER_1),
  UnifiedCenter: polarToCartesian(0, LAYOUT_RADII.POWER_CENTER_2),
};
