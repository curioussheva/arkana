...
export function getElementStats(
  counts: Record<ArcanaDefinition['element'], number>,
): ElementStats {
...
export function getElementAdvice(
  element: ArcanaDefinition['element'],
): string {
  return ELEMENT_ADVICE[element];
}

export function getElementOpening(
  element: ArcanaDefinition['element'],
): string {

  const openings =
    ELEMENT_OPENINGS[element];
...
export function buildElementSummary(
  ...
  return {
    ...
    advice: getElementAdvice(
      stats.dominant,
    ),

    opening: getElementOpening(
      stats.dominant,
    ),
  };
}