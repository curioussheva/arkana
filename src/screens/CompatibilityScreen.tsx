...
export function formatMaskedDate(text: string, prevText: string): string {
  ...
  // 2. Jika pengguna sedang menghapus (backspace), izinkan penghapusan tanpa auto-format
...
export function CompatibilityScreen() {
  ...
  // Helper pemetaan warna level
  const getLevelColor = (level: CompatibilityResult['level']): string => {
    switch (level) {
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <TouchableOpacity
            ...
            disabled={isCalculating || birthDate2.length < 10}
            activeOpacity={0.8}
          >
        ...
        {result && compositeResult && (
          ...
          >
            ...
            <Text style={[styles.level, { color: getLevelColor(result.level) }]}>
              {result.level}
            </Text>
            ...
            <View style={styles.elementRowContainer}>
              <View style={[styles.elementBadge, { backgroundColor: colors.backgroundLight }]}>
                ...
                <Text style={[styles.elementBadgeValue, { color: colors.primary }]}>
                  {result.dominantElements.person1}
                </Text>
              </View>
              <Text style={{ color: colors.textMuted, fontWeight: 'bold' }}>×</Text>
              <View style={[styles.elementBadge, { backgroundColor: colors.backgroundLight }]}>
                <Text style={[styles.elementBadgeLabel, { color: colors.textMuted }]}>
                  Pasangan
                </Text>
                <Text style={[styles.elementBadgeValue, { color: colors.primary }]}>
                  {result.dominantElements.person2}
                </Text>
              </View>
            </View>
            ...
            >
              <Text style={[styles.unionBoxTitle, { color: colors.primary }]}>
                🔮 Arcana Pusat Komposit: Titik E ({compositeResult.compositePoints['E']})
              </Text>
              <Text style={[styles.unionSoulName, { color: colors.text }]}>
                {compositeResult.interpretation.soulOfUnion}
              </Text>
              ...
              <Text style={[styles.unionText, { color: colors.textSecondary }]}>
                {compositeResult.interpretation.partnershipStrength}
              </Text>

              <Text
                style={[styles.sectionSubtitle, { color: colors.error, marginTop: SPACING.xs }]}
              >
                ⚠️ Tantangan Karma Bersama:
              </Text>
              <Text style={[styles.unionText, { color: colors.textSecondary }]}>
                {compositeResult.interpretation.karmicChallenge}
              </Text>

              <Text style={[styles.sectionSubtitle, { color: '#fbbf24', marginTop: SPACING.xs }]}>
                🔑 Nasihat Pertumbuhan:
              </Text>
              <Text style={[styles.unionText, { color: colors.textSecondary }]}>
                {compositeResult.interpretation.growthAdvice}
              </Text>
            </View>

            <Text
              style={[styles.narrative, { color: colors.textSecondary, marginTop: SPACING.md }]}
            >
              {result.narrative}
            </Text>
            ...
            {result.sharedArcanas.length > 0 && (
              <View style={styles.sharedArcanasContainer}>
                <Text style={[styles.detailsTitle, { color: colors.text, textAlign: 'left' }]}>
                  ✨ Arcana yang Beresonansi:
                </Text>
                ...
                {result.details.map((detail, idx) => (
                  ...
                  >
                    ...
                    <Text style={[styles.detailScore, { color: colors.primary }]}>
                      {detail.score}
                      <Text style={{ color: colors.textMuted }}>/{detail.maxScore}</Text>
                    </Text>
                  </View>
                ))}
              </View>
            ...
            <TouchableOpacity
              ...
              onPress={shareResult}
            >
              <Text style={{ color: colors.primary, fontWeight: '700', fontSize: FONT_SIZE.md }}>
                📤 Bagikan Hasil Energi
              </Text>
            </TouchableOpacity>
          </Animated.View>
        )}
