...
export const ArcanaWheel = forwardRef<any, ArcanaWheelProps>(function ArcanaWheel(
  ...
  const nodes = useMemo(() => {
    if (isMatrixMode && matrix) {
      ...
      return EVOLUTION_ORDER.map((key, index) => {
        ...
        const label =
          rawPoint.value !== undefined
            ? rawPoint.value.toString()
            : (arcana?.id?.toString() ?? '0');
  ...
  return (
    <View
      style={[
        styles.container,
        { backgroundColor: colors.backgroundLight + '40', borderColor: colors.border },
      ]}
    >
      {isMatrixMode && config && (
      ...
      <Canvas style={{ width: size, height: size }}>
        ...
        {!!dynamicPath && (
          <Path path={dynamicPath} style="stroke" strokeWidth={2} color={dynamicLineColor} />
        )}
          ...
          return (
            <Fragment key={`skia-${node.key}`}>
              ...
              <Circle
                ...
                style="fill"
                color={
                  isNodeLocked ? colors.backgroundLight : highlighted ? nodeColor : colors.surface
                }
              />
              <Circle
                cx={node.x}
